...
- **Transcripción Inteligente e Híbrida**: Utiliza Gemini en la nube o el motor nativo de Chrome (Web Speech API) para dictado en tiempo real sin límites de cuota local.
...
- Por defecto usa Gemini para el análisis y transcripción mediante *MediaRecorder*.
- Si activas **LM Studio** en la interfaz:
  - El micrófono utilizará el **Reconocimiento de Voz Nativo de Chrome** (Web Speech API) para transcribir el audio a texto de forma 100% gratuita y en tiempo real, sin consumir la cuota diaria de Gemini.
  - LM Studio se encargará del **análisis clínico** y la generación de la siguiente pregunta de la entrevista.
  - **Requisito de red:** Asegúrate de tener LM Studio corriendo en `http://127.0.0.1:1234`. La aplicación utiliza un **Proxy Inverso en su configuración de Vite (`/api-lmstudio`)** para evadir bloqueos de CORS o restricciones de Firewall del IMSS, enrutando el tráfico interno sin que el navegador bloquee la petición local.
